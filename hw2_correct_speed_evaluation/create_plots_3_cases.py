...
def plotTravelTimes(time_base, time_case1, time_case2, time_case3):
    #### travel time
    plt.figure()
    ...
    medians = np.round(np.asarray([np.median(data[0]), np.median(data[1]), np.median(data[2]), np.median(data[3])]), 2)
    ...
    plt.xticks(ticks=[0,1,2,3], labels=["Baseline", "Case 1", "Case 2", "Case 3"])
    plt.show()
    
def plotTravelDist(dist_base, dist_case1, dist_case2, dist_case3):
    #### travel distance
    plt.figure()
...
def plotPtWait(wait_base, wait_case1, wait_case2, wait_case3):
    #### pt wait times
    plt.figure()
    data = [np.ravel(wait_base), np.ravel(wait_case1), np.ravel(wait_case2), np.ravel(wait_case3)]
    boxplot = sns.boxplot(data = data, showfliers=False)
    medians = np.round(np.asarray([np.median(data[0]), np.median(data[1]), np.median(data[2]), np.median(data[3])]), 2)
    vertical_offset = np.median(medians) * 0.05 # offset from median for display
    for xtick in boxplot.get_xticks(): # display median on plot as number
...
def plotModeShare(mode_base, mode_case1, mode_case2, mode_case3):
    #### mode share
...
def plotTimePerKm(time_base, dist_base, time_case1, dist_case1, time_case2, dist_case2, time_case3, dist_case3):
    base_time_per_km = []
    ...
    #### minutes per km
    plt.figure()
    data = [np.ravel(base_time_per_km), np.ravel(case1_time_per_km), np.ravel(case2_time_per_km), np.ravel(case3_time_per_km)]
    boxplot = sns.boxplot(data = data, showfliers=False)
    medians = np.round(np.asarray([np.median(data[0]), np.median(data[1]), np.median(data[2]), np.median(data[3])]), 2)
    vertical_offset = np.median(medians) * 0.05 # offset from median for display
    for xtick in boxplot.get_xticks(): # display median on plot as number
...
def plotTimePerKmCarOnly(trips_base, trips_case1, trips_case2, trips_case3):
    #print("F")
    ...
    plt.figure()
    data = [time_per_km_base, time_per_km_case1, time_per_km_case2, time_per_km_case3]
    boxplot = sns.boxplot(data = data, showfliers=False)
    medians = np.round(np.asarray([np.median(data[0]), np.median(data[1]), np.median(data[2]), np.median(data[3])]), 2)
    vertical_offset = np.median(medians) * 0.05 # offset from median for display
    for xtick in boxplot.get_xticks(): # display median on plot as number
...
plotTravelTimes(travel_time_base, travel_time_case1, travel_time_case2, travel_time_case3)
plotTravelDist(travel_dist_base, travel_dist_case1, travel_dist_case2, travel_dist_case3)
plotPtWait(pt_wait_base, pt_wait_case1, pt_wait_case2, pt_wait_case3)
plotModeShare(modes_base, modes_case1, modes_case2, modes_case3)
plotTimePerKm(travel_time_base, travel_dist_base, travel_time_case1, travel_dist_case1, travel_time_case2, travel_dist_case2,travel_time_case3, travel_dist_case3)
plotTimePerKmCarOnly(trips_base, trips_case1, trips_case2, trips_case3)